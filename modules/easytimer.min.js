...
import { timeisup, hideAll } from '../index.js';
...
const visualElem = document.querySelector('.visual-object')
const textElem = document.querySelector('.text-object');
const ringsMarkerElem = document.querySelector('.ring-timer-marker')
let currentRing = "0"
// ANTAGLIGEN HÄR FELET LIGGER
const ringsElemNth = document.querySelector(`.ring:nth-child(${currentRing})`)



let ringsMinuteTimeLeft = ""
let tiotalMinuter = ""
let entalMinuter = ""
let tiotalSekunder = ""
let entalSekunder = ""
...
const number = [
    {spelled:"ETT", number: "1"},
    {spelled:"TVÅ", number: "2"},
    {spelled:"TRE", number: "3"},
    {spelled:"FYRA", number: "4"},
    {spelled:"FEM", number: "5"},
    {spelled:"SEX", number: "6"},
    {spelled:"SJU", number: "7"},
    {spelled:"ÅTTA", number: "8"},
    {spelled:"NIO", number: "9"}
];
const numberTen = [
    {spelled:"ELVA", number: "1"},
    {spelled:"TOLV", number: "2"},
    {spelled:"TRETTON", number: "3"},
    {spelled:"FJORTON", number: "4"},
    {spelled:"FEMTON", number: "5"},
    {spelled:"SEXTON", number: "6"},
    {spelled:"SJUTTON", number: "7"},
    {spelled:"ARTON", number: "8"},
    {spelled:"NITTON", number: "9"}
];
let timerValue = 10;
let arrayNumbers = []
...
function startTimer() {
    let timer = new easytimer.Timer({countdown: true, startValues: {seconds: timerValue*60}});
    let timerAnalog = new easytimer.Timer({precision: 'seconds', startValues:{seconds: 0}, target: {seconds: timerValue*60}});
    let timerVisual = new easytimer.Timer({precision: 'seconds', startValues:{seconds: 0}, target: {seconds: timerValue*60}});
    digitalTimerElem.innerHTML = timer.getTimeValues().toString();

    timer.start();
    timerAnalog.start();
    timerVisual.start();
    ringsGenerate(timerValue);
    
    
    timerVisual.addEventListener('secondsUpdated', function (e) {
        const currentSecondsVisual = (timerAnalog.getTimeValues().seconds);
        const currentMinutesVisual = ((timerAnalog.getTimeValues().minutes)*60);
        const currentHoursVisual = ((timerAnalog.getTimeValues().hours)*60*60);
        const totalVisual = currentSecondsVisual+currentMinutesVisual+currentHoursVisual;
        const percentage =(totalVisual/(timerValue*60))*100;
        visualElem.style.height = `${percentage}%`;
        
    });

    timerAnalog.addEventListener('secondsUpdated', function (e) {
        const secondsDegrees = (timerAnalog.getTimeValues().seconds * 6);
        secondHand.style.transform = `rotate(${secondsDegrees}deg)`;
        const minsDegrees = ((timerAnalog.getTimeValues().minutes) *6);
        minsHand.style.transform = `rotate(${minsDegrees}deg)`;
    });
    timerAnalog.addEventListener('targetAchieved', function (e) {
        timeisup();
    });
    ...
    timer.addEventListener('secondsUpdated', function (e) {
        ...
        let textString = timer.getTimeValues().toString()
        let textFiltered = textString.slice(-5);
 
        
        for (let i = 0; i < textFiltered.length; i++) {
            const number = textFiltered[i];
        ...
        for (let i = 0; i < arrayNumbers.length; i++) {
            let numberPosition = arrayNumbers[i];

            if (numberPosition== arrayNumbers[0]){
                tiotalMinuter = numberPosition;
                for(let j = 0; j < number.length; j++) {
                    if(number[j].number == tiotalMinuter){
                        document.querySelector('#minutes-ten').innerText = `${number[j].spelled}TIO`;
                    }
                }  
            } if (numberPosition== arrayNumbers[1]){
                entalMinuter = numberPosition;
                for(let j = 0; j < number.length; j++) {
                    if(number[j].number == entalMinuter){
                        document.querySelector('#minutes').innerHTML = `${number[j].spelled} MINUTER <br>OCH `;
                    }
                }   
            } if (numberPosition ==arrayNumbers[2]){
                tiotalSekunder = numberPosition;
                for(let j = 0; j < number.length; j++) {
                    if(number[j].number == tiotalSekunder){
                        document.querySelector('#seconds-ten').innerText = `${number[j].spelled}TIO`;
                    }
                }   
            } if (numberPosition ==arrayNumbers[3]){
                entalSekunder = numberPosition;
                for(let j = 0; j < number.length; j++) {
                    if(number[j].number == entalSekunder){
                        document.querySelector('#seconds').innerText = `${number[j].spelled}`;
                    }
                }   
            }
        }
        arrayNumbers= [];
               ...
               ///RINGS UPDATE LOGIK
               ringsMinuteTimeLeft = textString.slice(-5,-3)
               // ANTAGLIGEN HÄR FELET LIGGER
            //    ringsElemNth.classList.add('.ringCurrent')
    });
    ...
    for (let i = 0; i < abortBtn.length; i++) {
        ...
        singleAbortBtn.addEventListener('click', ()=> {
            ...
            timerVisual.reset();
            timerVisual.stop();
            document.querySelector('#minutes-ten').innerText ="" ;
            document.querySelector('#minutes').innerText = "";
            document.querySelector('#seconds-ten').innerText ="" ;
            document.querySelector('#seconds').innerText ="" ;

            // LOGIK FÖR ATT TA BORT RINGAR
            // for (let j = 0; j <= timerValue; j++) {
            //     const targetDeleteRing = document.querySelector('.ring:nth-child(j)');
            //     targetDeleteRing.remove;                
            // }
        }) 
    }
...
function ringsGenerate(ringsNumbers){

for (let i = 0; i < ringsNumbers; i++) {
let divCreate = document.createElement("div")
divCreate.classList.add('ring');
ringsMarkerElem.appendChild(divCreate);
}
